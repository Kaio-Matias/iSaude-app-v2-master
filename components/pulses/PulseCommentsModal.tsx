...
import { PulseComment, pulseCommentsService } from './PulseCommentsService';
...
export default function PulseCommentsModal({ visible, pulseId, onClose, onCommentAdded }: PulseCommentsModalProps) {
  ...
  useEffect(() => {
    const loadComments = async () => {
      try {
        const loadedComments = await pulseCommentsService.getComments(pulseId);
  ...
  const handleAddComment = async () => {
    ...
    setLoading(true);
    try {
      const comment = await pulseCommentsService.addComment(
        pulseId,
        newComment.trim(),
        currentUser.name,
        currentUser.avatar
      ...
      setNewComment('');
      onCommentAdded(); // Atualiza o contador no pulse
    } catch {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const success = await pulseCommentsService.deleteComment(commentId, pulseId);
            if (success) {
  ...
  const renderComment = ({ item }: { item: PulseComment }) => (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>Seja o primeiro a comentar neste pulse!</Text>
            </View>
          }
        />
